...
TL;DR: 提出LLM-Verifier收敛定理，为LLM辅助的形式验证提供首个具有可证明终止和收敛保证的正式框架，通过马尔可夫链建模和大量实验验证了理论预测。


<details>
  <summary>Details</summary>
Motivation: 当前使用形式验证工具与大型语言模型（LLM）的方法虽然扩展了软件验证能力，但缺乏理论基础，导致验证过程不可靠、不稳定，需要建立具有可证明保证的正式框架。

Method: 将LLM与验证器的交互建模为离散时间马尔可夫链，关键参数为错误减少概率δ，推导出LLM-Verifier收敛定理，并通过超过90,000次试验进行压力测试验证理论预测。

Result: 理论证明对于任何δ>0，程序几乎必然终止，期望迭代次数有界𝔼[n]≤4/δ。实验结果显示所有运行都达到验证，收敛因子紧密聚集在Cf≈1.0，理论与实际行为高度一致。
...
TL;DR: Flowchart2Mermaid：一个将流程图图像转换为可编辑Mermaid.js代码的轻量级网页系统，支持混合主动式精炼和AI助手


<details>
  <summary>Details</summary>
Motivation: 流程图作为常见的流程沟通工具，通常以静态图像形式分享，难以编辑和重用。现有工具缺乏生成结构化、版本可控的文本表示方法。

Method: 使用详细系统提示和视觉语言模型，将流程图图像转换为Mermaid.js代码。界面支持混合主动式精炼：内联文本编辑、拖放节点插入、自然语言命令（由集成AI助手解释）。

Result: 开发了一个轻量级网页系统，能够生成结构化、版本可控的文本表示，并与渲染的图表保持同步。引入了评估指标来评估多个模型的结构准确性、流程正确性、语法有效性和完整性。

Conclusion: 与之前的图像到图表工具不同，该方法产生结构化、版本可控的文本表示，保持与渲染图表的同步，并通过评估指标验证了系统性能。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界模型通常计算成本高，但真实场景往往包含模块化交互的子组件。为了在AI安全所需的透明性和实际推理所需的计算效率之间架起桥梁，需要开发可分解的世界模型框架。

Method: 开发一个框架，将transducer（一种推广POMDP的模型类）表示的复杂世界模型分解为在独立输入输出子空间上运行的子transducer。该方法反转了transducer组合过程，实现可并行化和可解释的替代方案。

Result: 该框架能够从复杂的transducer世界模型中推导出子transducer，支持分布式推理，为模块化世界建模提供了理论基础。

Conclusion: 这项工作为连接AI安全所需的结构透明性和实际推理所需的计算效率奠定了基础，提供了替代整体世界建模的并行化、可解释方案。
...
TL;DR: STRIDE框架帮助决定何时使用自主AI代理而非简单LLM调用，通过任务分解和动态性评估提供AI部署建议，减少45%不必要的代理部署并节省37%资源成本。


<details>
  <summary>Details</summary>
Motivation: 随着从无状态大语言模型向自主目标驱动代理的快速转变，需要解决核心问题：何时真正需要自主AI代理？盲目部署代理会导致成本、复杂性和风险增加，因此需要系统化方法来决定何时使用简单LLM调用、引导式助手还是完全自主代理。

Method: 提出STRIDE框架，通过结构化任务分解、动态性归因和自我反思需求分析，计算代理适用性分数，为任务推荐三种AI部署模式：直接LLM调用、引导式AI助手、完全自主代理。

Result: 在30个真实世界任务（SRE、合规和企业自动化）中评估，STRIDE达到92%的模式选择准确率，减少45%不必要的代理部署，降低37%资源成本。6个月专家验证确认其实际效用。

Conclusion: 将代理采用重新定义为必要性驱动的设计决策，确保自主性仅在收益证明成本合理时使用。STRIDE为AI部署提供原则性指导，避免过度使用复杂代理系统。
...
TL;DR: 该论文扩展了TheAgentCompany平台，创建了一个专注于财富管理的金融环境，研究通用LLM智能体是否能准确且经济地完成代表性财富管理任务，并开发了包含12个任务对的基准测试来评估智能体的适用性。


<details>
  <summary>Details</summary>
Motivation: 尽管现代工作依赖各种数字协作工具，但常规流程仍然受到人为错误和延迟的影响。研究旨在填补这一空白，探索通用LLM智能体在财富管理领域的应用潜力，特别是能否准确且经济地完成助理级别的财富管理工作。

Method: 扩展TheAgentCompany平台，创建金融专用环境；引入合成领域数据；丰富同事模拟；原型化自动任务生成管道；构建包含12个任务对的财富管理助理基准测试，涵盖检索、分析和综合/沟通任务，每个任务都有明确的验收标准和确定性评分器；为每个任务设计了高自主性和低自主性两种变体。

Result: 研究发现智能体的限制主要不在于数学推理能力，而在于端到端工作流程的可靠性；自主性水平对智能体性能有显著影响；不正确的模型评估方法阻碍了基准测试的有效性。

Conclusion: 通用LLM智能体在财富管理任务中面临的主要挑战是工作流程可靠性而非数学能力，自主性水平是影响性能的关键因素，需要改进评估方法以更准确地衡量智能体在财富管理领域的适用性。
...
TL;DR: TradeTrap：一个统一的评估框架，用于系统性地压力测试自适应和程序化自主交易代理，揭示其在系统级扰动下的脆弱性。


<details>
  <summary>Details</summary>
Motivation: LLM驱动的交易代理在真实金融市场中部署越来越多，但其在对抗性或故障条件下的可靠性和鲁棒性尚未得到充分检验，而金融环境具有高风险、不可逆的特点。

Method: 提出TradeTrap框架，针对自主交易代理的四个核心组件（市场情报、策略制定、投资组合与账本处理、交易执行）进行系统级扰动测试，在真实美股市场数据的闭环历史回测环境中进行公平可复现的比较。

Result: 实验表明，单个组件的小扰动会通过代理决策循环传播，导致极端集中、失控敞口和大幅投资组合回撤，证明当前自主交易代理在系统层面易被误导。

Conclusion: 当前自主交易代理存在系统性脆弱性，TradeTrap框架为评估和改进交易代理的鲁棒性提供了重要工具，有助于提高金融AI系统的可靠性。
...
TL;DR: 该论文指出当前AI系统在自我监控、元认知、适应性学习、目标重构等方面存在七大核心缺陷，主张需要向基于认知原理的AI（认知自主性）进行范式转变，以实现真正的自主性和适应性。


<details>
  <summary>Details</summary>
Motivation: 尽管AI在感知、语言、推理和多模态领域取得了快速进展，但现代AI系统在自我监控、自我纠正和动态环境中自主调节行为方面仍然存在根本性限制。这些限制阻碍了AI实现稳健的泛化、终身适应性和真实世界的自主性。

Method: 通过比较分析人工系统与生物认知，整合AI研究、认知科学和神经科学的见解，识别并分析了当代AI模型的七大核心缺陷。同时提出了基于神经认知原理的架构发展方向。

Result: 识别出七大核心缺陷：缺乏内在自我监控、缺乏元认知意识、固定和非适应性学习机制、无法重构目标、缺乏表征维护、不足的具身反馈、缺乏内在能动性。这些缺陷无法通过单纯扩展模型规模来解决。

Conclusion: 主张向认知自主性AI进行范式转变，这种AI能够进行自我导向的适应、动态表征管理和有意图的目标导向行为，同时配备改革性监督机制，确保自主系统保持可解释性、可治理性并与人类价值观对齐。
...
TL;DR: DialogGuard是一个多智能体框架，用于评估LLM在心理健康、危机干预等敏感场景中的心理社会风险，包括隐私侵犯、歧视行为、心理操纵、心理伤害和侮辱行为五个维度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）越来越多地应用于心理健康、危机干预等情感敏感服务，但其在这些场景中的心理社会安全性评估不足，缺乏有效的评估框架。

Method: 提出DialogGuard多智能体框架，包含四种LLM-as-a-judge评估流程：单智能体评分、双智能体校正、多智能体辩论和随机多数投票，基于共享的三级评估标准，适用于人类标注者和LLM评估者。

Result: 在PKU-SafeRLHF数据集上验证，多智能体机制比非LLM基线和单智能体评估更准确地检测心理社会风险；双智能体校正和多数投票在准确性、与人类评分对齐和鲁棒性方面达到最佳平衡；辩论机制召回率更高但会过度标记边界案例。

Conclusion: DialogGuard作为开源软件发布，提供每个维度的风险评分和可解释的自然语言推理，支持从业者进行提示设计、审计和监督面向脆弱用户的网络应用。
...
TL;DR: MERINDA是一个FPGA加速的模型恢复框架，用并行化神经架构替代迭代求解器，在边缘设备上实现高效、实时的自主系统决策。


<details>
  <summary>Details</summary>
Motivation: 模型恢复（MR）对于关键任务自主系统的安全可解释决策很重要，但当前基于神经ODE的迭代求解方法在FPGA上效率低下，内存和能耗问题阻碍了其在边缘设备上的实时部署。

Method: 提出MERINDA框架，用并行化神经架构替代神经ODE的迭代求解器，专门针对FPGA优化，减少内存访问和计算开销。

Result: 相比移动GPU，MERINDA实现了近11倍的DRAM使用降低和2.2倍的运行速度提升，在固定精度下展示了内存与能耗的逆相关关系。

Conclusion: MERINDA特别适合资源受限、需要实时操作的关键任务自主系统，为边缘设备上的模型恢复部署提供了高效解决方案。
...
TL;DR: 仅用599张训练图像实现95.5% Dice分数的乳腺细胞分割框架，通过量子启发的边缘增强、稳定化多组件损失函数和复杂度加权采样策略，显著减少医学图像标注所需专家时间。


<details>
  <summary>Details</summary>
Motivation: 医学图像标注需要大量时间和专家知识，特别是乳腺上皮细胞核数据集的标注需要病理学家投入数百小时。这成为临床感知AI发展的主要瓶颈，需要开发能够在有限标注数据下实现高性能的方法。

Method: 1) 使用多尺度Gabor滤波器进行量子启发边缘增强，创建第四输入通道改善边界检测；2) 稳定化多组件损失函数，整合自适应Dice损失、边界感知项和自动正样本加权，处理严重类别不平衡；3) 复杂度加权采样策略优先处理具有挑战性的乳腺上皮细胞区域；4) EfficientNet-B7/UNet++架构配合4到3通道投影，利用预训练权重；5) 指数移动平均和统计异常值检测进行鲁棒验证。

Result: 在乳腺细胞分割任务中达到95.5% ± 0.3%的Dice分数和91.2% ± 0.4%的IoU。量子增强贡献了2.1%的边界精度提升，加权采样使小病变检测提高了3.8%。仅使用599张训练图像（其中仅4%像素代表乳腺组织，60%图像不含乳腺区域）。

Conclusion: 该框架通过创新的量子启发边缘增强、稳定化损失函数和智能采样策略，在有限标注数据下实现了突破性性能，显著减少了医学专家创建数据集所需时间，解决了临床感知AI发展的根本瓶颈。
...
TL;DR: OmniGuard是一个面向全模态大语言模型的安全护栏系统，通过结构化安全标签和专家模型蒸馏，在文本、图像、视频、音频等多种模态上提供统一的安全保障。


<details>
  <summary>Details</summary>
Motivation: 全模态大语言模型（OLLMs）处理文本、图像、视频和音频等多种模态，带来了新的安全挑战。现有安全护栏研究主要针对单模态场景，通常将安全保障简化为二元分类，这在多样模态和任务中缺乏鲁棒性。

Method: 提出OmniGuard框架，构建包含超过21万个多样化样本的全模态安全数据集，涵盖所有模态的单模态和跨模态样本。每个样本都带有结构化安全标签，并通过目标蒸馏从专家模型获取精心策划的安全评估。

Result: 在15个基准测试上的广泛实验表明，OmniGuard在各种多模态安全场景中表现出强大的有效性和泛化能力。该系统为全模态提供了统一的政策执行和风险缓解框架。

Conclusion: OmniGuard是首个具有深思熟虑推理能力的全模态安全护栏家族，为构建更鲁棒和强大的全模态安全保障系统铺平了道路。
...
Method: 开发ReMindView-Bench基准测试，系统性地变化视角空间模式和查询类型来探测空间认知的关键因素；使用LLM-as-a-judge和自一致性提示进行显式阶段分析，以及线性探测和熵动态进行隐式分析。

Result: 评估15个当前VLM显示在跨视角对齐和视角转换方面存在一致失败；模型在帧内感知表现良好，但在跨视角信息整合时性能急剧下降；隐式分析显示任务相关信息逐渐丢失，正确与错误轨迹之间的不确定性分离。

Conclusion: ReMindView-Bench为VLM空间推理提供了认知基础的诊断，揭示了多视角空间心智模型在推理过程中如何形成、退化和失稳，为改进模型的空间推理能力提供了方向。
...
TL;DR: 使用LLM驱动的生成式智能体构建MMO游戏仿真系统，通过SFT和RL训练实现真实玩家行为模拟，为数值设计和机制优化提供低成本、高保真的离线测试框架。


<details>
  <summary>Details</summary>
Motivation: 传统MMO游戏数值系统优化依赖大规模在线实验或基于预定义统计模型的参数调优，成本高、耗时长且可能影响玩家体验。现有离线仿真系统保真度有限，无法准确模拟真实玩家的推理和干预反应。

Method: 提出基于大语言模型的生成式智能体MMO仿真系统：1）使用大规模真实玩家行为数据对LLM进行监督微调和强化学习，将通用先验适配到游戏特定领域；2）基于真实游戏日志训练数据驱动的环境模型，重建动态游戏系统。

Result: 实验表明系统与真实世界玩家行为保持高度一致性，在干预下产生合理的因果响应，为数据驱动的数值设计优化提供了可靠、可解释且成本高效的框架。

Conclusion: LLM驱动的生成式智能体仿真系统能够准确模拟玩家决策，提供高保真、可解释的离线测试环境，显著降低MMO游戏数值设计和机制优化的成本与风险。
...
TL;DR: 研究探索使用监督学习配合人工错误注入来培养语言模型的自我纠正能力，但发现该方法效果有限，无法显著提升性能，且存在分布偏移问题。


<details>
  <summary>Details</summary>
Motivation: 强化学习已成为激发大语言模型推理和自我纠正能力的主要方法，但其计算成本高昂，促使研究者探索替代方案。受自动驾驶和机器人技术启发，本研究旨在验证监督学习结合人工错误注入能否有效培养语言模型的自我纠正能力。

Method: 在推理链中插入人工错误，将其掩码，然后通过监督学习训练模型识别和纠正这些错误。该方法试图通过合成错误覆盖来模拟真实错误场景。

Result: 该方法即使在简单的合成任务上也无法显著提升多个模型的性能。即使模型发现了自己的错误，也常常重复原始错误。研究发现，从合成错误到在线策略错误的分布偏移显著降低了微调模型的错误纠正能力，即使合成错误覆盖了在线策略错误。

Conclusion: 研究结果解释了为什么在线策略强化学习方法在激发自我纠正能力方面具有独特效果。监督学习结合人工错误注入的方法虽然直观，但受限于分布偏移问题，效果有限。
...
TL;DR: AI代理自动聚类预测市场并识别市场间的依赖关系（正相关/负相关），构建交易策略获得约20%周度收益


<details>
  <summary>Details</summary>
Motivation: 预测市场存在碎片化问题，包括重叠问题、隐含等价性和隐藏矛盾，需要自动发现市场间的语义结构关系

Method: 使用基于自然语言理解的AI代理管道：1）根据合约文本和元数据将市场聚类到主题组；2）识别组内市场对之间的结果依赖关系（相同结果相关和不同结果负相关）

Result: 在Polymarket历史数据上，AI识别的关系准确率达60-70%，基于这些关系构建的交易策略在周度时间范围内获得约20%的平均收益

Conclusion: AI代理和大语言模型能够有效发现预测市场中潜在的语义结构，将关系识别转化为可操作的交易信号
...
TL;DR: R-Few是一个引导式自演化的Challenger-Solver框架，通过少量人工标注的上下文指导和混合训练，使AI模型能够稳定可控地自我进化，在数学和通用推理任务上实现持续迭代改进。


<details>
  <summary>Details</summary>
Motivation: AI自我进化长期以来被视为通往超智能的路径，但实践中无指导的自演化系统往往快速达到平台期甚至性能退化，主要问题包括概念漂移、多样性崩溃和错误进化，模型会强化自身偏见并收敛到低熵行为。

Method: 提出R-Few框架，包含引导式自演化的Challenger-Solver架构：Challenger使用少量人工标注示例指导合成问题生成，Solver在在线难度课程下联合训练人工和合成示例，通过上下文基础化和混合训练融入轻量级人工监督。

Result: 在数学和通用推理基准测试中，R-Few实现了一致且迭代的改进：Qwen3-8B-Base在数学任务上比R-Zero提升+3.0分，性能与General-Reasoner相当，而后者使用了20倍的人工数据。消融研究证实了基础化Challenger训练和课程化Solver训练的互补贡献。

Conclusion: R-Few通过轻量级人工监督的引导式自演化框架，有效缓解了概念漂移问题，产生了更稳定可控的协同进化动态，为实现AI模型的稳定自我进化提供了可行路径。
...
TL;DR: COPE是一个基于思维链的轻量级开源框架，利用LLaMA-3-8B模型从非结构化临床笔记预测急性缺血性卒中90天功能结局，性能与GPT-4.1相当，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 临床笔记包含丰富的上下文信息，但其非结构化特性限制了在传统预测模型中的应用。需要开发能够利用这些信息进行准确预后预测的解决方案。

Method: 开发了两步思维链框架COPE：第一步生成临床推理，第二步输出改良Rankin量表预测。基于464名患者的出院小结和90天mRS评分，使用LLaMA-3-8B开源模型，并与GPT-4.1、ClinicalBERT、结构化机器学习模型和单步LLM进行比较。
...
<details>
  <summary>Details</summary>
Motivation: 数字内容的指数增长给内容安全带来重大挑战。现有的基于单一模型或固定流水线的审核系统在识别隐式风险和提供可解释判断过程方面存在局限性。

Method: 提出Aetheria框架，采用五个核心智能体的协作架构，通过基于RAG知识检索的动态相互说服辩论机制，对多模态内容进行深度分析和裁决。

Result: 在提出的AIR-Bench基准测试中，Aetheria不仅生成详细可追溯的审计报告，而且在整体内容安全准确性上显著优于基线方法，特别是在隐式风险识别方面。
...
TL;DR: 提出了一种结合视频、音频和文本的多模态共情预测方法，利用监督文档作为特权信息辅助训练，在训练阶段使用LDA模型约束文本特征，实验证明优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有共情预测技术主要集中于单一模态（通常是文本），忽略了多模态处理能力，并且没有利用某些特权信息（可能包含额外的共情内容）。
...
Conclusion: 提出的多模态共情预测方法结合了视频、音频和文本信息，并通过监督文档辅助训练有效利用了特权信息，显著提升了共情预测性能。
...
TL;DR: PaperDebugger：一款基于多代理的LaTeX编辑器插件，将LLM驱动的学术写作助手直接嵌入编辑环境，支持上下文感知的文档操作和修订管理。


<details>
  <summary>Details</summary>
Motivation: 现有学术写作助手与编辑器分离，无法深度交互文档状态、结构和修订历史，限制了在LaTeX编辑器（如Overleaf）中进行上下文感知的代理操作。

Method: 通过Chrome扩展、Kubernetes原生编排层和模型上下文协议（MCP）工具链实现，包含可靠的编辑器双向同步、细粒度版本控制、安全状态管理、多代理调度和外部工具集成。

Result: 展示了完全集成的工作流，包括本地化编辑、结构化评审、并行代理执行和基于差异的更新，早期聚合分析显示用户积极参与。

Conclusion: PaperDebugger验证了编辑器原生、代理驱动的学术写作助手的实用性，将LLM推理直接带入写作环境。
...
TL;DR: IACT是一种通过用户对话驱动的动态递归代理拓扑模型，可自主扩展组织复杂度以匹配开放式任务，通过双向对话减少错误传播。
...
Method: 基于用户对话驱动的通用自主系统，自主构建动态递归代理拓扑，用双向状态化对话替代刚性函数调用，实现运行时错误纠正和歧义解决。

Result: 在kragent.ai系统中成功部署，通过实际工作流提供定性证据，而非详尽基准测试结果。

Conclusion: IACT模型通过动态递归代理拓扑和双向对话机制，有效解决了传统代理系统的局限性，实现了更灵活、容错性更强的自主系统。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据驱动的RUL预测方法通常假设训练和测试数据来自相同分布，但实际工业环境中存在域差异问题。先前对抗域自适应方法专注于提取域不变特征，但忽略了目标特定信息和退化阶段的一致性特征。

Method: 提出TACDA方法：1) 在对抗自适应过程中加入目标域重构策略，在保持目标特定信息的同时学习域不变特征；2) 开发新颖的聚类和配对策略，实现相似退化阶段之间的一致性对齐。

Result: 通过大量实验验证，TACDA方法在两种不同评估指标上均显著优于现有最先进方法。

Conclusion: TACDA通过结合目标域重构和退化阶段一致性对齐，有效解决了跨域RUL预测中的域差异问题，在保持目标特定信息的同时实现了更好的性能。
...
TL;DR: 提出一种基于图神经网络编码布尔公式序列的新方法，用于强化学习代理执行任意LTL指令，解决了多事件同时发生时的复杂交互问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理多个高级事件同时发生且可能复杂交互的环境中存在不足，需要一种能够处理任意LTL指令且能应对复杂交互的多任务策略学习方法。

Method: 将LTL指令解释为有限自动机，策略以简单布尔公式序列为条件，这些公式直接对应自动机中的转移，通过图神经网络编码生成结构化任务表示。

Result: 在复杂的基于象棋的环境中进行的实验证明了该方法的优势，能够有效处理多事件同时发生的复杂交互场景。

Conclusion: 提出的基于图神经网络编码布尔公式序列的方法能够有效学习执行任意LTL指令的多任务策略，解决了现有方法在处理复杂事件交互时的局限性。
...
TL;DR: 论文研究了LLM在递归推理问题中的深度泛化限制，提出了一种循环定位-替换管道方法来解决嵌套层次结构问题。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在许多任务上表现出色，但在处理需要解决嵌套层次结构的递归推理问题时面临显著挑战。现有研究主要关注长度泛化（处理比训练时更长的序列），而本文研究一个未被充分探索的限制：深度泛化，即模型处理比训练时更深嵌套层次的能力。

Method: 提出了一种新颖的循环定位-替换管道方法，将递归问题分解为可管理的子组件。该方法使用两个专门模型：定位器识别可解的子表达式，替换器评估这些组件同时保持整体结构。

Result: 在布尔代数、递归算术和命题逻辑三个精心设计的领域中评估该方法，每个领域都有可控的递归深度。结果表明，该方法在测试超出分布范围的递归深度时，有效缓解了性能衰减问题。
...
TL;DR: MIGR框架通过模态重要性引导推理，解决多模态情感理解中的推理漂移问题，提高解释的可靠性
...
Method: 提出模态重要性机制识别情感主导模态，重组推理序列从关键模态开始；采用两阶段框架：模态对齐监督微调和模态感知奖励优化

Result: 在DFEW基准上，MIGR显著提高推理可靠性，将正确预测但情感不一致解释的比例从18.10%降至7.37%

Conclusion: 从情感主导模态开始推理能有效提高多模态情感理解的可靠性，模态重要性引导的推理框架具有实际应用价值
...
TL;DR: Menta是一个专门针对社交媒体心理健康预测优化的轻量级小语言模型，通过多任务学习在抑郁、压力、自杀倾向等任务上表现优异，比非微调的SLMs平均提升15.2%，甚至在某些任务上超过130亿参数的大模型，同时能在iPhone等设备上实时部署。


<details>
  <summary>Details</summary>
Motivation: 心理健康问题影响全球数亿人，但早期检测仍然有限。虽然大语言模型在心理健康应用中有潜力，但其规模和计算需求阻碍了实际部署。小语言模型提供了轻量级替代方案，但在基于社交媒体的心理健康预测方面研究不足。

Method: 开发Menta模型：1) 使用LoRA框架进行多任务微调；2) 采用跨数据集训练策略；3) 使用平衡准确率导向的损失函数；4) 在六个分类任务上联合训练，涵盖抑郁、压力和自杀倾向等心理健康问题。

Result: 1) 相比性能最佳的非微调SLMs，在各项任务上平均提升15.2%；2) 在抑郁和压力分类任务上，准确率超过130亿参数的大语言模型；3) 模型大小约为大模型的3.25倍小；4) 可在iPhone 15 Pro Max上实时部署，仅需约3GB内存。

Conclusion: Menta展示了优化的小语言模型在心理健康监测方面的潜力，能够实现可扩展、隐私保护的实时监测，为轻量级心理健康预测系统提供了可行方案。
...
TL;DR: StockMem：一个事件-反思双层记忆框架，通过结构化新闻事件和追踪事件演化来提升股价预测的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 股价预测面临市场波动性和对实时事件的敏感性挑战。虽然大语言模型为基于文本的预测提供了新途径，但在金融领域的应用受到新闻数据噪声和文本中缺乏明确答案的限制。通用记忆架构难以识别价格变动的关键驱动因素。

Method: 提出StockMem框架，将新闻结构化为事件，并从两个维度挖掘：横向整合整合每日事件，纵向追踪捕捉事件演化以提取反映市场预期差异的增量信息，构建时序事件知识库。通过分析事件-价格动态，进一步形成因果经验反思知识库。预测时检索类似历史场景，并结合当前事件、增量数据和过去经验进行推理。

Result: 实验表明StockMem优于现有记忆架构，通过追踪影响价格的信息链提供更优、可解释的推理，增强了金融预测的决策透明度。

Conclusion: StockMem通过事件-反思双层记忆框架有效解决了金融新闻数据噪声和事件演化追踪问题，提升了股价预测的准确性和可解释性，为金融预测提供了更透明的决策支持。
...
<details>
  <summary>Details</summary>
Motivation: 传统日记账测试(JETs)产生过多误报且难以检测细微异常，需要更有效的异常检测方法
...
TL;DR: 将心理测量学框架扩展到动态系统，提出GVU算子生成参数流，推导出保证自改进稳定性的方差不等式，并用该框架统一了多种自改进架构。


<details>
  <summary>Details</summary>
Motivation: 现有AAI能力评分是静态函数，需要将代理建模为计算资源参数化的动态系统，以理解自改进的稳定性和机制。

Method: 形式化代理为GVU算子生成的参数流，证明该算子生成参数流形上的向量场，推导出保证自改进稳定性的方差不等式（谱条件）。

Result: 建立了自改进的数学框架，证明当生成和验证的组合噪声足够小时（考虑曲率和步长效应），自改进系数κ>0，并用该框架统一了多种自改进架构。

Conclusion: 提出的GVU算子和方差不等式为理解自改进系统提供了统一的数学框架，解释了多种现有架构的成功机制，并为设计稳定自改进系统提供了理论指导。
...
TL;DR: 提出基于因果概念的后解释XAI框架，通过概念干预的充分概率生成局部和全局解释，在CelebA数据集上验证，强调解释的可理解性和忠实性


<details>
  <summary>Details</summary>
Motivation: 为不可解释模型提供既易于理解又忠实于原模型的解释框架，解决当前XAI中可理解性与忠实性难以兼顾的问题

Method: 基于因果概念的后解释方法，通过计算概念干预的充分概率来生成局部和全局解释，使用CelebA数据集构建概念验证模型

Result: 展示了基于清晰概念词汇的解释示例，验证了框架的可理解性，并通过强调重要假设确保解释的忠实性

Conclusion: 提出的因果概念后解释XAI框架能够生成既易于理解又忠实于模型的解释，但需要确保解释生成与解释解释的上下文一致
...
<details>
  <summary>Details</summary>
Motivation: 现有自动化论文复现框架缺乏对每个生成步骤输出的验证和精炼机制，或过度依赖人工设计的提示词进行自我精炼，限制了适应性和可扩展性

Method: 提出无需提示词的协作智能体框架，包含验证智能体（检查每个步骤输出是否满足系统提示要求）和精炼智能体（基于识别问题修订输出），仅利用原始系统提示实现自动验证和改进

Result: 在PaperBench Code-Dev和Paper2CodeBench数据集上实验表明，该方法显著提升复现代码的准确性和完整性，相比无智能体的基线分别提升约15%和13%，且比Self-Refine方法更具鲁棒性和一致性

Conclusion: 提出的提示词无关协作智能体框架能有效自动提升论文到代码生成质量，解决了现有方法依赖人工设计提示词的局限性，具有更好的适应性和可扩展性
...
TL;DR: Radiologist Copilot是一个基于大语言模型的智能助手，通过协调多个工具实现自动化放射学报告生成与质量控制，模拟放射科医生完整工作流程，显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 放射学报告撰写是临床检查中耗时且易出错的任务，特别是对于三维医学图像。现有自动化方法主要关注报告生成阶段，忽略了关键的质量控制流程，无法为放射科医生提供全面支持。

Method: 利用大语言模型作为推理核心，构建智能代理系统，自主选择工具、规划和执行动作。协调的工具包括：区域定位、基于"think with image"范式的区域分析规划、报告生成策略模板选择、质量评估和反馈驱动的自适应优化。

Result: 实验结果表明，Radiologist Copilot在放射学报告生成方面显著超越了其他最先进的方法，能够提供准确、完整和高效的放射学报告。

Conclusion: Radiologist Copilot通过模拟放射科医生完整工作流程，实现了自动化放射学报告生成与质量控制，能够辅助放射科医生并提高临床效率。源代码将在论文接受后发布。
...
<details>
  <summary>Details</summary>
Motivation: 尽管投资增加，但过去二十年新矿发现减少，需要解决关键矿产勘探效率低下的问题

Method: 基于贝叶斯主义和证伪原则的哲学方法，将数据获取视为证伪人类假设的手段，使用可验证指标进行理性决策，结合无监督学习和人机协作AI

Result: 提出实用协议模板，通过AI驱动的科学方法减少认知偏差，优化数据采集决策

Conclusion: AI能够实现严格的科学方法，减少勘探中的认知偏差和假阳性，提高发现效率并降低成本
...
TL;DR: 本文提出基于鞅属性的无监督评估框架，用于检测LLM推理中的信念固化现象，发现当前信念能正向预测未来信念更新，表明存在确认偏误而非理性贝叶斯更新。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM推理能力提升，但迭代推理可能导致信念固化和确认偏误，而非增强求真行为。需要系统评估LLM是否进行理性贝叶斯更新。

Method: 利用贝叶斯统计中的鞅属性（未来信念期望值等于当前信念），提出无监督、基于回归的鞅分数来衡量违反该属性的程度，信号偏离贝叶斯更新能力。

Result: 在事件预测、价值负载问题和学术论文评审等开放领域，发现违反鞅属性现象普遍存在，当前信念能正向预测未来信念更新（信念固化）。识别了易受影响的模型、推理技术和领域。

Conclusion: 鞅分数能预测有标签领域的真实准确性，表明其作为无监督指标能有效评估推理过程的求真能力，揭示了LLM推理中普遍存在的信念固化问题。
